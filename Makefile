...
# Shell configuration - use bash for all recipes (required for [[ ]], =~, &>, etc.)
SHELL := /bin/bash
...
help:
	@echo -e "$(BLUE)LazySSH Development Makefile$(NC)"
	@echo "=============================="
	@echo ""
	@echo -e "$(YELLOW)Note: After 'make install' or 'make dev-install', run: source venv/bin/activate$(NC)"
	@echo ""
	...
	@echo ""
	@echo -e "$(GREEN)Build & Release Commands:$(NC)"
	...
	@echo ""
	@echo -e "$(GREEN)Documentation Commands:$(NC)"
	@echo "  make docs             Verify documentation files exist"
	@echo ""
	@echo -e "$(GREEN)Composite Commands:$(NC)"
	@echo "  make all              Run all checks and tests (verify + coverage)"
	@echo ""
	@echo -e "$(YELLOW)Tip: Run 'make fix' before 'make check' to auto-fix most issues$(NC)"
...
install:
	@echo -e "$(BLUE)Setting up development environment for $(PACKAGE_NAME)...$(NC)"
	@if [ ! -d "$(VENV_DIR)" ]; then \
...
deps-update:
	...
	@echo -e "$(YELLOW)⚠ This will update all dependencies. Consider reviewing changes.$(NC)"
...
venv-info:
	@if [ -d "$(VENV_DIR)" ]; then \
		...
		echo -e "$(BLUE)To activate it, run:$(NC)"; \
		echo -e "  $(YELLOW)source $(VENV_DIR)/bin/activate$(NC)"; \
		echo ""; \
...
fmt:
	@echo -e "$(BLUE)Formatting code with Black and isort...$(NC)"
...
fix: fmt
	...
	@echo -e "$(YELLOW)→ Running pyupgrade for Python 3.11+...$(NC)"
	...
	@echo -e "$(YELLOW)→ Re-running formatters after pyupgrade...$(NC)"
	$(ISORT) $(ISORT_OPTS) $(SRC_DIR) $(TEST_DIR)
	$(BLACK) $(BLACK_OPTS) $(SRC_DIR) $(TEST_DIR)
	@echo -e "$(GREEN)✓ Auto-fix complete$(NC)"
	@echo -e "$(YELLOW)→ Run 'make check' to verify all issues are resolved$(NC)"
...
lint:
	...
	@echo -e "$(YELLOW)→ Running flake8 (syntax errors)...$(NC)"
	@$(FLAKE8) $(SRC_DIR) $(TEST_DIR) --count --select=E9,F63,F7,F82 --show-source --statistics || \
		(echo -e "$(RED)✗ Flake8 found critical errors$(NC)" && exit 1)
	@echo -e "$(YELLOW)→ Running flake8 (full check)...$(NC)"
...
check: 
	...
	@$(BLACK) --check $(BLACK_OPTS) $(SRC_DIR) $(TEST_DIR) || \
		(echo -e "$(RED)✗ Black formatting check failed. Run 'make fix' to auto-fix$(NC)" && exit 1)
	@$(ISORT) --check-only $(ISORT_OPTS) $(SRC_DIR) $(TEST_DIR) || \
		(echo -e "$(RED)✗ isort check failed. Run 'make fix' to auto-fix$(NC)" && exit 1)
	@echo -e "$(GREEN)✓ Format check passed$(NC)"
	@echo ""
	@echo -e "$(BLUE)=== Step 2/5: Linting ===$(NC)"
	@$(MAKE) lint
	@echo ""
	@echo -e "$(BLUE)=== Step 3/5: Type Checking ===$(NC)"
	@$(MAKE) typecheck
	@echo ""
	@echo -e "$(BLUE)=== Step 4/5: Import Test ===$(NC)"
	@$(PYTHON) -c "import lazyssh; print(f'Successfully imported lazyssh version {lazyssh.__version__}')" || \
		(echo -e "$(RED)✗ Package import failed$(NC)" && exit 1)
	@echo -e "$(GREEN)✓ Package import successful$(NC)"
	@echo ""
	@echo -e "$(BLUE)=== Step 5/5: Pathlib Check ===$(NC)"
	@OSPATH_COUNT=$$(grep -r "os\.path\." --include="*.py" $(SRC_DIR) 2>/dev/null | wc -l | tr -d ' '); \
	if [ "$$OSPATH_COUNT" -gt 0 ]; then \
		echo -e "$(YELLOW)⚠ Found $$OSPATH_COUNT instances of os.path usage (consider using pathlib)$(NC)"; \
		grep -r "os\.path\." --include="*.py" $(SRC_DIR) | head -5; \
	else \
		echo -e "$(GREEN)✓ No os.path usage found$(NC)"; \
	fi
	@echo ""
...
verify:
	@echo -e "$(BLUE)═══════════════════════════════════════════════════════════════$(NC)"
	@echo -e "$(BLUE)  COMPREHENSIVE VERIFICATION (PyPI-ready checks)                $(NC)"
	@echo -e "$(BLUE)═══════════════════════════════════════════════════════════════$(NC)"
	@echo ""
	@if [ ! -f "pre-commit-check.sh" ]; then \
		echo -e "$(RED)✗ pre-commit-check.sh not found$(NC)"; \
		exit 1; \
	fi
	...
	@echo -e "$(GREEN)  Repository is ready for commit/push/deploy to PyPI           $(NC)"
	@echo -e "$(GREEN)═══════════════════════════════════════════════════════════════$(NC)"
...
test:
	...
	@if [ -d "$(TEST_DIR)" ] && [ "$$(find $(TEST_DIR) -name 'test_*.py' | wc -l)" -gt 0 ]; then \
		$(PYTEST) $(PYTEST_OPTS) $(TEST_DIR) || (echo -e "$(RED)✗ Tests failed$(NC)" && exit 1); \
...
coverage:
	...
	@if [ -d "$(TEST_DIR)" ] && [ "$$(find $(TEST_DIR) -name 'test_*.py' | wc -l)" -gt 0 ]; then \
		...
		echo -e "$(YELLOW)→ Open htmlcov/index.html to view the report$(NC)"; \
	else \
		echo -e "$(YELLOW)⚠ No test files found in $(TEST_DIR)$(NC)"; \
	fi
...
clean:
	@echo -e "$(BLUE)Cleaning up build artifacts and virtual environment...$(NC)"
...
publish-test: verify build
	...
	$(TWINE) check dist/* || (echo -e "$(RED)✗ Package verification failed$(NC)" && exit 1)
	$(TWINE) upload --repository-url https://test.pypi.org/legacy/ dist/* || \
		(echo -e "$(RED)✗ Upload to TestPyPI failed$(NC)" && exit 1)
	@echo -e "$(GREEN)✓ Successfully published to TestPyPI$(NC)"

publish: verify build
	@echo -e "$(RED)⚠⚠⚠ WARNING: Publishing to PyPI (production) ⚠⚠⚠$(NC)"
	...
	$(TWINE) check dist/* || (echo -e "$(RED)✗ Package verification failed$(NC)" && exit 1)
	$(TWINE) upload dist/* || (echo -e "$(RED)✗ Upload to PyPI failed$(NC)" && exit 1)
...
all: verify coverage
	@echo ""
	@echo -e "$(GREEN)✓✓✓ All checks and tests completed successfully! ✓✓✓$(NC)"
