...
### Requirement: Terminal Method User Feedback - Scenario: Change terminal method at runtime via command

**Reason**: This scenario is being replaced by the modified version above that clarifies the `terminal` command is now dedicated to method switching only.

**Migration**: The functionality remains the same - users still use `terminal <method>` to change methods. The only change is that the command no longer has dual functionality (it no longer opens terminals for connections).
